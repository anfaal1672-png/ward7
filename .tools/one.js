...
const D=+process.argv[2], sd=+process.argv[3], CAP=+(process.env.CAP||800);
A.toTitle(); A.skipUI(true); A.seed(sd); A.botOn(true); A.settings.diff=D; A.start(); H.pump(2);
const p=A.player;
let hp=100, hits=0, f=0;
while(A.state()===2 && f<CAP*60){ H.pump(30); f+=30; if(p.hp<hp){ hits++; hp=p.hp; } }
const st=A.state();
let stage;
if(p.got < p.need) stage='カルテ'+p.got+'/'+p.need;
else if(!p.hasKey) stage='鍵';
else stage='出口へ';
